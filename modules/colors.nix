...
#
# Uses Tokyo Night as the base16 palette family with two variants:
#   - Tokyo Night Dark — used by config.colorScheme.palette
#   - Tokyo Night Light — exposed via lightPalette
#
# The upstream nix-colors base16 mapping has broken base08/base09/base0A
# assignments, so both palettes are defined manually.
#
# Other modules reference colors through config.colorScheme.palette.baseXX.
# The light palette is exposed for editor theme variants.
{
  inputs,
  lib,
  ...
  ...
  # Per-app theme toggle registry. Each module declares how to switch its
  # theme at runtime so the toggle script and activation hooks stay in sync
  # without hardcoding app-specific paths in hyprland.nix.
  ...
  config.colorScheme = {
    ...
    palette = {
      ...
      base0D = "7AA2F7"; # blue — functions, methods, links
