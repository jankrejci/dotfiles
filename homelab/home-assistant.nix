...
#
# Zigbee stack:
#   Nabu Casa ZBT-2 coordinator -> zigbee2mqtt -> mosquitto MQTT -> Home Assistant
#
# Data flow:
...
in {
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "homelab.home-assistant requires homelab.postgresql.enable = true";
      }
      {
    ...
    # Order after hard and soft dependencies
    ...
    homelab.dashboardEntries = [
      {
        name = "HomeAssistant";
        url = "https://${haDomain}";
        icon = ../assets/dashboard-icons/homeassistant.svg;
      }
    ];
