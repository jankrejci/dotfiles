...
# - required for probe-rs with supported debug probes
#
# Upstream: https://probe.rs/files/69-probe-rs.rules
#
# probe-rs detects its rule file by searching /etc/udev/rules.d for a
# filename containing "probe-rs". Install via services.udev.packages so
# the filename is preserved instead of merged into a single rules file.
{pkgs, ...}: {
  ...
  services.udev.packages = [
    (pkgs.writeTextFile {
      ...
      text = ''
        ...
        # ARM DAPLink (CMSIS-DAP) compatible debug probes
